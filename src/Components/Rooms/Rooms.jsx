...
const Rooms = () => {
  ...
  useEffect(() => {
    axiosSecure.get(`/rooms?sortRooms=${sortRooms}`).then((res) => {
      ...
      >
        ...
        <div className="bg-[#ffffff] dark:bg-slate-900 py-10 lg:py-20">
          <div className="w-[90vw] m-auto flex flex-col lg:flex-row justify-center gap-5">
            <div className="flex flex-col p-20  h-[10vw] lg:w-[20vw] lg:h-[60vh] lg:bg-[#212538] text-white rounded-xl mb-10 lg:mb-0">
              <label htmlFor="range" className="text-3xl font-Edu">
                Filter Rooms
              </label>
              <select
                onChange={handleFilter}
                name="range"
                className="px-4 py-2 lg:py-2  bg-blue-100 rounded-md outline-none text-black font-semibold"
              >
            ...
            <div className="lg:w-[70vw] grid  md:grid-cols-2 lg:grid-cols-3 gap-y-7 lg:gap-y-14">
              {roomsData?.map((i) => (
